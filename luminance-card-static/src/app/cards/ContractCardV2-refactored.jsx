...
const ContractCard = ({ context, actions }) => {
  ...
  useEffect(() => {
    const fetchDealData = async () => {
      try {
        ...
        if (properties.hs_attachment_ids) {
          try {
            const attachmentIds = properties.hs_attachment_ids.split(';');
            setAttachments(attachmentIds.map(id => ({ id, name: `Attachment ${id}` })));
          } catch (err) {
  ...
  // Execute contract generation - REFACTORED: No external fetch
  const executeGenerate = async () => {
    ...
    try {
      console.log('🚀 Updating deal properties to trigger workflow...');
      console.log('Deal ID:', dealId);
      console.log('Portal ID:', portalId);
      console.log('Contract Type:', formData.contractType);
      console.log('Action:', 'generate');

      // Update deal properties directly - workflow will detect this change
      const propertiesToUpdate = {
        luminance_trigger_action: formData.actionType,
        luminance_contract_type: formData.contractType,
        luminance_trigger_timestamp: Date.now().toString(),
        luminance_notes: formData.notes || '',
        luminance_contract_status: 'processing' // Reset status
      };

      console.log('📤 Updating properties:', JSON.stringify(propertiesToUpdate, null, 2));

      await actions.updateCrmObjectProperties(propertiesToUpdate);

      console.log('✅ Deal properties updated successfully');
      console.log('⏳ Workflow should now trigger and call Prismatic...');
      console.log('⏳ Polling for contract URL in deal properties...');

      setStatusMessage('Contract generation triggered. Waiting for Prismatic to process...');
      setPolling(true); // Start polling

    } catch (err) {
      console.error('Error updating deal properties:', err);
  ...
  // Execute contract upload - REFACTORED: No external fetch
  const executeUpload = async () => {
    ...
    try {
      ...
      const propertiesToUpdate = {
        luminance_trigger_action: formData.actionType,
        luminance_contract_type: formData.contractType,
        luminance_attachment_id: selectedFile,
        luminance_trigger_timestamp: Date.now().toString(),
        luminance_contract_status: 'processing' // Reset status
      };
      ...
      setStatusMessage('Contract upload triggered. Waiting for Prismatic to process...');
      setPolling(true); // Start polling
